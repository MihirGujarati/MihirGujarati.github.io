...
function BulldozerIcon({ mode, isDark }: { mode: 'up' | 'down'; isDark: boolean }) {
  // Blade positions
  ...
  // Colors from SVG files
  const trackColor = isDark ? '#FFED4E' : '#FFD700';
  const bodyColor = isDark ? '#FF9500' : '#FF8C00';

  return (
    <svg width="36" height="36" viewBox="0 0 64 64" fill="none" aria-hidden="true">
      {/* Tracks */}
      <rect x="10" y="44" width="38" height="10" rx="5" stroke={trackColor} strokeWidth="4" />
      <circle cx="18" cy="49" r="2.2" fill={trackColor} opacity="0.9" />
      <circle cx="26" cy="49" r="2.2" fill={trackColor} opacity="0.9" />
      <circle cx="34" cy="49" r="2.2" fill={trackColor} opacity="0.9" />
      <circle cx="42" cy="49" r="2.2" fill={trackColor} opacity="0.9" />

      {/* Body */}
      <rect x="16" y="30" width="26" height="14" rx="4" stroke={bodyColor} strokeWidth="4" />
      {/* Cab */}
      <rect x="22" y="22" width="14" height="10" rx="3" stroke={bodyColor} strokeWidth="4" />
      <path d="M26 24h6" stroke={bodyColor} strokeWidth="3" strokeLinecap="round" opacity="0.9" />
      ...
      />
      {/* Blade teeth */}
      <path d={`M52 ${bladeBottom} L54 ${bladeBottom + 4}`} stroke={trackColor} strokeWidth="3" strokeLinecap="round" />
      <path d={`M55 ${bladeBottom} L57 ${bladeBottom + 4}`} stroke={trackColor} strokeWidth="3" strokeLinecap="round" />
      <path d={`M58 ${bladeBottom} L60 ${bladeBottom + 4}`} stroke={trackColor} strokeWidth="3" strokeLinecap="round" />
    </svg>
  );
}
...
export default function CustomCursor() {
  ...
  return (
    <>
      ...
      {/* Bulldozer cursor */}
      <motion.div
        className={`fixed top-0 left-0 pointer-events-none z-50 hidden sm:block ${isDark ? 'mix-blend-difference' : ''}`}
        animate={{
          x: position.x - 18,
          y: position.y - 18,
      ...
      >
        <div className={`${isDark ? 'shadow-[0_0_18px_rgba(255,255,255,0.35)]' : ''} rounded-xl p-1`}>
          <BulldozerIcon mode={mode} isDark={isDark} />
        </div>
      </motion.div>
